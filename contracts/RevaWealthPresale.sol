...
/*
 RevaWealth Investment Solution                                       
*/
...
contract RevaWealthPresale is Ownable {
    ...
    modifier saleEnabled() {
        require(saleStatus == true, "RevaWealthIS: is not enabled");
        _;
    }

    modifier saleStoped() {
        require(saleStatus == false, "RevaWealthIS: is not stopped");
        _;
    }
    ...
    function setSaleToken(
        ...
        require(_rate != 0);
        rate = _rate;
        initialRate = _rate; // Store initial rate for dynamic pricing
    ...
    function toggleDynamicPricing(bool _enabled) external onlyOwner {
        ...
        emit DynamicPricingToggled(_enabled, block.timestamp);
    }

    function calculateDynamicRate() public view returns (uint256) {
        if (!dynamicPricingEnabled || totalTokensSold == 0) {
            return rate;
        }

        // Calculate percentage of tokens sold
        uint256 soldPercentage = (totalTokensSold * 100) / totalTokensforSale;
        
        // Dynamic rate calculation based on tokens sold
        // Rate increases as more tokens are sold (price goes up)
        if (soldPercentage >= 80) {
            // 80-100% sold: 50% increase from initial rate
            return initialRate + (initialRate * 50) / 100;
        } else if (soldPercentage >= 60) {
            // 60-79% sold: 30% increase from initial rate
            return initialRate + (initialRate * 30) / 100;
        } else if (soldPercentage >= 40) {
            // 40-59% sold: 15% increase from initial rate
            return initialRate + (initialRate * 15) / 100;
        } else if (soldPercentage >= 20) {
            // 20-39% sold: 5% increase from initial rate
            return initialRate + (initialRate * 5) / 100;
        } else {
            // 0-19% sold: initial rate
            return initialRate;
        }
    }

    function updateRateBasedOnSales() external onlyOwner {
        require(dynamicPricingEnabled, "RevaWealthIS: Dynamic pricing not enabled");
        
        uint256 oldRate = rate;
        uint256 newRate = calculateDynamicRate();
        
        if (newRate != oldRate) {
            rate = newRate;
            emit RateUpdated(oldRate, newRate, totalTokensSold, block.timestamp);
        }
    }
    ...
    function addPayableTokens(
        ...
        require(
            ...
            "RevaWealthIS: tokens & prices arrays length mismatch"
        );
    ...
    function updateTokenRate(
        ...
        // Update ETH rate if provided
        if (_rate != 0) {
            uint256 oldRate = rate;
            rate = _rate;
            
            // If this is the first time setting the rate, also set initial rate
            if (initialRate == 0) {
                initialRate = _rate;
            }
            
            emit RateUpdated(oldRate, _rate, totalTokensSold, block.timestamp);
        }

        // Update token prices
        for (uint256 i = 0; i < _tokens.length; i += 1) {
            require(payableTokens[_tokens[i]] == true, "RevaWealthIS: Token not in payable list");
            require(_prices[i] != 0, "RevaWealthIS: Price cannot be zero");
            tokenPrices[_tokens[i]] = _prices[i];
        }
    }
    ...
    function getTokenAmount(
        ...
        if (token != address(0)) {
            require(
                ...
                "RevaWealthIS: Token not allowed"
            );
    ...
    function buyToken(
        ...
        require(saleTokenAmt != 0, "RevaWealthIS: Amount is 0");
        require(
            (totalTokensSold + saleTokenAmt) < totalTokensforSale,
            "RevaWealthIS: Not enough tokens to be sold"
        );
        ...
        // Update rate based on new total tokens sold if dynamic pricing is enabled
    ...
    function buyersDetailsList(
        ...
        require(_from < _to, "RevaWealthIS: _from should be less than _to");
    ...
    function getPricingInfo() external view returns (
        ...
        currentRate = rate;
        soldPercentage = (totalTokensSold * 100) / totalTokensforSale;
        
        if (soldPercentage >= 80) {
            currentTier = "Tier 5 (80-100% sold) - 50% increase";
        } else if (soldPercentage >= 60) {
            currentTier = "Tier 4 (60-79% sold) - 30% increase";
        } else if (soldPercentage >= 40) {
            currentTier = "Tier 3 (40-59% sold) - 15% increase";
        } else if (soldPercentage >= 20) {
            currentTier = "Tier 2 (20-39% sold) - 5% increase";
        } else {
            currentTier = "Tier 1 (0-19% sold) - Initial rate";
        }
    }
    ...
    function withdrawFunds(address token, uint256 amount) external onlyOwner {
        if (token == address(0)) {
            ...
            require(
                balance >= amount,
                "RevaWealthIS: Insufficient Ether balance"
            );
            ...
            require(
                balance >= amount,
                "RevaWealthIS: Insufficient token balance"
            );
